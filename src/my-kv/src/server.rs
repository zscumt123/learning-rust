...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        let stream: tokio_rustls::server::TlsStream<tokio::net::TcpStream> =
            tls.accept(stream).await?;
        let stream: ProstServerStream<tokio_rustls::server::TlsStream<tokio::net::TcpStream>> =
            ProstServerStream::new(stream, service.clone());
